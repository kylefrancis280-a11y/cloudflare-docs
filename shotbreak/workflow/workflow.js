...
function getCrewSpecsFor(step, p){
  ...
  switch (step) {
    case 'vision': {
      ...
      return [
        { id: 'logline-refiner',      label: 'Logline Refiner',      credits: credOf('logline-refiner'),      input: build('Sharpen the logline to a single irresistible sentence. Identify the protagonist, the stakes, the antagonist force, and the genre promise. Return refined_logline, logline_notes, genre_signal.') },
        { id: 'premise-expander',     label: 'Premise Expander',     credits: credOf('premise-expander'),     input: build('Expand the logline into a full premise document: theme, central conflict, emotional core, world rules, and the question the film must answer. Return premise_statement, theme, central_conflict, emotional_core, world_rules, key_question.') },
        { id: 'research-specialist',  label: 'Research Specialist',  credits: credOf('research-specialist'),  input: build('Research the genre, reference films, and tonal anchors. Identify what makes each reference film work, extract craft lessons applicable to this project. Return reference_analysis[], craft_lessons[], genre_conventions[], avoid_cliches[].') },
        { id: 'genre-interpreter',    label: 'Genre Interpreter',    credits: credOf('genre-interpreter'),    input: build('Decode the genre contract — what the audience expects, what conventions must be honored, which can be subverted for effect. Return genre_tags[], conventions[], violations[], audience_expectations[], subversion_opportunities[].') },
      ];
    }
    case 'story': {
      const build = (instr) => JSON.stringify({...commonScriptInput, instruction: instr}, null, 2);
      return [
        ...
        { id: 'dialogue-editor',             label: 'Dialogue Coach',              credits: credOf('dialogue-editor'),             input: build('Score voice distinctness per character, propose targeted rewrites for lines that fail.') },
        ...
        { id: 'creative-prompt-writer',      label: 'Format Auditor',              credits: credOf('creative-prompt-writer'),      input: build('List industry-standard format issues (slug case, cue placement, paragraph length, transitions). Do not rewrite content.') },
        { id: 'creative-director',           label: 'Subtext Director',            credits: credOf('creative-director'),           input: build('Layer subtext — name per-scene subtextual_intent, character_undertones, and action-line enrichments that carry meaning wordlessly.') },
        ...
        { id: 'character-designer',          label: 'Voice Architect',             credits: credOf('character-designer'),          input: build('Lock voice signatures across the 6 dimensions per character — vocabulary register, sentence length, contractions, patterns, never-says, regional markers.') },
        ...
        { id: 'dramatic-logic-guardian',     label: 'Dramatic Logic Guardian',     credits: credOf('dramatic-logic-guardian'),     input: build('Audit every scene for dramatic logic failures — unmotivated character decisions, plot holes, cause-effect breaks. Return flags[] with scene_ref, issue, severity, suggested_fix.') },
        { id: 'logline-refiner',             label: 'Story Logline Check',         credits: credOf('logline-refiner'),             input: build('Verify the logline still accurately reflects the script as written. Flag any drift between logline promise and story delivery. Return verdict, drift_notes, refined_logline if needed.') },
        { id: 'feedback-integrator',         label: 'Feedback Integrator',         credits: credOf('feedback-integrator'),         input: build('Synthesize all story-layer agent outputs into a unified story health report. Prioritize the top 5 issues by impact. Return health_score, priority_issues[], quick_wins[], structural_verdict.') },
      ];
    }
    case 'cast': {
      const build = (instr) => JSON.stringify({...commonScriptInput, instruction: instr}, null, 2);
      return [
        ...
        { id: 'character-arc-guardian',        label: 'Arc Guardian',                  credits: credOf('character-arc-guardian'),        input: build('Build actionable psychology per character — core wound through arc trajectory.') },
        ...
        { id: 'character-designer',            label: 'Voice Signature Lock',          credits: credOf('character-designer'),            input: build('Lock voice signatures across 6 dimensions per character.') },
        ...
        { id: 'world-builder',                 label: 'Architecture Designer',         credits: credOf('world-builder'),                 input: build('Per-location structural specs — building type, era, materials, scale, architectural signature.') },
        { id: 'makeup-hair-specialist',        label: 'Makeup & Hair Specialist',      credits: credOf('makeup-hair-specialist'),        input: build('Design makeup and hair continuity per character — base look, scene-specific variations, aging/injury progressions, period accuracy notes. Return character_looks[] with scene_overrides[].') },
        { id: 'stunt-coordinator',             label: 'Stunt Coordinator',             credits: credOf('stunt-coordinator'),             input: build('Identify all stunt sequences in the script. Rate complexity, flag safety considerations, suggest stunt double requirements, and note VFX assist opportunities. Return stunts[] with scene_ref, complexity, safety_flags[], vfx_assist.') },
        { id: 'practical-effects-supervisor',  label: 'Practical Effects Supervisor',  credits: credOf('practical-effects-supervisor'),  input: build('Identify all practical effects needed — pyrotechnics, rain, breakaways, animatronics. Rate feasibility for AI video generation vs real-world shoot. Return effects[] with scene_ref, type, feasibility, ai_prompt_note.') },
        { id: 'crowd-coordinator',             label: 'Crowd Coordinator',             credits: credOf('crowd-coordinator'),             input: build('Identify all crowd/background scenes. Specify crowd size, behavior, period accuracy, and AI generation prompt strategy for background population. Return crowd_scenes[] with scene_ref, size, behavior, prompt_strategy.') },
        { id: 'dop-assistant',                 label: 'DOP Assistant',                 credits: credOf('dop-assistant'),                 input: build('Assist the Director of Photography — compile lens choices, camera body recommendations, filter stack, and shooting format per scene type. Return dop_notes[] with scene_ref, lens, format, filter, rationale.') },
        { id: 'wardrobe-coordinator',          label: 'Wardrobe Coordinator',          credits: credOf('wardrobe-coordinator'),          input: build('Design wardrobe continuity per character — canonical outfit, scene variations, period/genre accuracy, color psychology alignment with character arc. Return wardrobe_plan[] with character, canonical_look, scene_overrides[], color_notes.') },
      ];
    }
    case 'coverage': {
      const build = (instr) => JSON.stringify({...commonScriptInput, shot_list: p.shot_list, instruction: instr}, null, 2);
      const leanBuild = (instr) => JSON.stringify({
        scenes: (commonScriptInput.scenes||[]).map(s=>({id:s.id,summary:s.summary,action:(s.action||'').substring(0,300)})),
        vision: commonScriptInput.vision,
        instruction: instr
      }, null, 2);
      return [
        ...
        { id: 'prop-master',                  label: 'Prop Master (Scene)',          credits: credOf('prop-master'),                  input: leanBuild('Per-scene hero + signature + active props with significance hierarchy.') },
        ...
        { id: 'lighting-designer',            label: 'Lighting Designer',            credits: credOf('lighting-designer'),            input: leanBuild('Design per-scene lighting plan — key light, fill, rim, motivated source, mood note, color temperature. Return lighting_plan[] with scene_ref and all fields.') },
        { id: 'movement-choreographer',       label: 'Movement Choreographer',       credits: credOf('movement-choreographer'),       input: build('Design camera movement per shot — dolly, handheld, crane, static, motivated movement rationale. Return movement_plan[] with shot_ref, movement_type, rationale.') },
        { id: 'location-scout',               label: 'Location Scout',               credits: credOf('location-scout'),               input: leanBuild('Finalize location descriptions and establishing shot prompts. Verify sensory anchors are AI-generatable. Return location_library[] with description, establishing_prompt, sensory_anchors[].') },
        { id: 'color-grading-agent',          label: 'Color Theorist',               credits: credOf('color-grading-agent'),          input: leanBuild('Design per-scene color palette and emotional intent. Define global palette rationale aligned with vision. Return color_plan[] with scene_ref, palette, intent, and global_palette_rationale.') },
        { id: 'dop-assistant',                label: 'DOP Coverage Notes',           credits: credOf('dop-assistant'),                input: build('Review shot list for coverage gaps — missing reverses, cutaways, inserts. Flag under-covered scenes. Return coverage_gaps[] with scene_ref, missing_shot_type, priority.') },
        { id: 'gaffer',                       label: 'Gaffer',                       credits: credOf('gaffer'),                       input: leanBuild('Plan electrical and lighting rig per scene — power requirements, practical light sources, LED panel placement, generator needs for AI generation reference. Return gaffer_plan[] with scene_ref, rig_notes, practical_sources[].') },
        { id: 'key-grip',                     label: 'Key Grip',                     credits: credOf('key-grip'),                     input: build('Plan grip equipment per shot — dolly track, crane arm, stabilizer, C-stand placement. Return grip_plan[] with shot_ref, equipment[], setup_notes.') },
        { id: 'sound-recordist',              label: 'Sound Recordist',              credits: credOf('sound-recordist'),              input: leanBuild('Plan on-set sound recording strategy — boom placement, lavalier needs, ambient sound capture, problem scenes (wind, traffic, crowd). Return sound_plan[] with scene_ref, mic_strategy, problem_flags[].') },
        { id: 'stunt-performer-coordinator',  label: 'Stunt Performer Coordinator',  credits: credOf('stunt-performer-coordinator'),  input: leanBuild('Map stunt performers to specific shots. Specify safety protocols, camera angles that protect performer identity, and AI generation notes for stunt sequences. Return stunt_assignments[] with shot_ref, performer_notes, safety_protocol, ai_note.') },
      ];
    }
    case 'generate': {
      const build = (instr) => JSON.stringify({...commonScriptInput, shot_list: p.shot_list, instruction: instr}, null, 2);
      const leanBuild = (instr) => JSON.stringify({
        scenes: (commonScriptInput.scenes||[]).map(s=>({id:s.id,summary:s.summary,action:(s.action||'').substring(0,300)})),
        shot_list: (p.shot_list||[]).map(s=>({id:s.id,scene_id:s.scene_id,prompt:s.prompt,model:s.model})),
        vision: commonScriptInput.vision,
        instruction: instr
      }, null, 2);
      return [
        { id: 'creative-prompt-writer',             label: 'Prompt Strategist',               credits: credOf('creative-prompt-writer'),             input: build('Set prompt strategy, model allocation across Kling/Veo/Hailuo/Seedance, global negative prompt, character reference policy (I2V vs T2V).') },
        { id: 'storyboard-artist',                  label: 'Shot Calibrator',                 credits: credOf('storyboard-artist'),                  input: build('Per-shot per-model variants — Kling/Veo/Hailuo/Seedance tuned prompts with rationale.') },
        ...
        { id: 'vfx-compositor',                     label: 'VFX Compositor',                  credits: credOf('vfx-compositor'),                     input: leanBuild('Design compositing plan for flagged VFX shots — layer order, alpha channel strategy, green screen notes, plate requirements. Return composite_plan[] with shot_ref, layers[], technique, complexity.') },
        { id: 'vfx-artist',                         label: 'VFX Artist',                      credits: credOf('vfx-artist'),                         input: leanBuild('Specify VFX artwork requirements per shot — matte paintings, digital set extensions, environment replacements. Return vfx_art[] with shot_ref, type, description, ai_generatable.') },
        { id: 'vfx-particle-specialist',            label: 'VFX Particle Specialist',         credits: credOf('vfx-particle-specialist'),            input: leanBuild('Design particle effects per shot — fire, smoke, dust, rain, sparks, magic. Specify density, behavior, color, and AI prompt strategy. Return particles[] with shot_ref, effect_type, prompt_strategy.') },
        { id: 'vfx-cgi-character-designer',         label: 'CGI Character Designer',          credits: credOf('vfx-cgi-character-designer'),         input: leanBuild('Design any fully CGI or hybrid characters — topology notes, texture strategy, rigging requirements, AI generation approach. Return cgi_characters[] with shot_ref, character, technique, prompt_notes.') },
        { id: 'vfx-motion-graphics-designer',       label: 'Motion Graphics Designer',        credits: credOf('vfx-motion-graphics-designer'),       input: leanBuild('Design motion graphics elements — title cards, UI screens, HUD overlays, text animations. Return motion_graphics[] with shot_ref, type, style, content, timing.') },
        { id: 'vfx-matte-painter',                  label: 'Matte Painter',                   credits: credOf('vfx-matte-painter'),                  input: leanBuild('Design matte painting requirements — background extensions, skyline replacements, period environment builds. Return matte_paintings[] with shot_ref, description, ai_prompt, complexity.') },
        { id: 'vfx-rotoscope-artist',               label: 'Rotoscope Artist',                credits: credOf('vfx-rotoscope-artist'),               input: leanBuild('Identify shots requiring rotoscoping — subject isolation, background replacement, edge refinement. Return roto_shots[] with shot_ref, subject, complexity, frame_count_estimate.') },
        { id: 'vfx-tracking-matchmove-specialist',  label: 'Tracking & Matchmove Specialist', credits: credOf('vfx-tracking-matchmove-specialist'),  input: leanBuild('Identify shots requiring camera tracking or matchmove — moving camera VFX integration, screen replacements, object tracking. Return tracking_shots[] with shot_ref, type, complexity.') },
        { id: 'vfx-lighting-integration-artist',    label: 'VFX Lighting Integration Artist', credits: credOf('vfx-lighting-integration-artist'),    input: leanBuild('Design lighting integration for VFX elements — match practical light sources, shadow casting, ambient occlusion strategy. Return lighting_integration[] with shot_ref, vfx_element, light_match_notes.') },
        { id: 'vfx-simulation-artist',              label: 'VFX Simulation Artist',           credits: credOf('vfx-simulation-artist'),              input: leanBuild('Design physics simulations — cloth, fluid, rigid body destruction, hair. Return simulations[] with shot_ref, sim_type, complexity, ai_assist_possible.') },
        { id: 'vfx-destruction-specialist',         label: 'VFX Destruction Specialist',      credits: credOf('vfx-destruction-specialist'),         input: leanBuild('Design destruction sequences — building collapse, vehicle crash, explosion aftermath. Return destruction_shots[] with shot_ref, type, scale, prompt_strategy.') },
        { id: 'vfx-wire-removal-artist',            label: 'VFX Wire Removal Artist',         credits: credOf('vfx-wire-removal-artist'),            input: leanBuild('Identify shots requiring wire/rig removal or digital cleanup. Return cleanup_shots[] with shot_ref, element_to_remove, complexity, technique.') },
        { id: 'vfx-final-deliverer',                label: 'VFX Final Deliverer',             credits: credOf('vfx-final-deliverer'),                input: leanBuild('Compile final VFX delivery checklist — all shots, completion status, render specs, codec requirements, naming conventions. Return delivery_checklist[] with shot_ref, status, specs.') },
      ];
    }
    case 'edit': {
      const build = (instr) => JSON.stringify({...commonScriptInput, shot_list: p.shot_list, timeline: p.timeline, instruction: instr}, null, 2);
      return [
        ...
        { id: 'head-editor',                    label: 'Head Editor',                   credits: credOf('head-editor'),                    input: build('Design the master edit strategy — assembly order, sequence logic, scene groupings, overall narrative rhythm. Return edit_strategy with assembly_order[], sequence_logic, polish_notes.') },
        { id: 'cut-specialist',                 label: 'Cut Specialist',                credits: credOf('cut-specialist'),                 input: build('Design precise cut points per scene — in-point, out-point, cut motivation, emotional beat served. Return cuts[] with scene_ref, in_point, out_point, motivation.') },
        { id: 'montage-specialist',             label: 'Montage Specialist',            credits: credOf('montage-specialist'),             input: build('Identify montage opportunities — parallel editing, time compression, thematic montage. Design shot sequence and rhythm. Return montages[] with scene_ref, type, shot_sequence[], rhythm_notes.') },
        { id: 'cross-fade-transition-artist',   label: 'Transition Designer',           credits: credOf('cross-fade-transition-artist'),   input: build('Design transitions between every scene — cut, dissolve, wipe, match cut, smash cut, fade. Return transitions[] with from_scene, to_scene, type, rationale.') },
        { id: 'sound-editor',                   label: 'Sound Editor',                  credits: credOf('sound-editor'),                   input: build('Design sound edit plan — dialogue cleanup, SFX placement, room tone, sync issues. Return sound_edit[] with scene_ref, issues[], sfx_placements[], sync_notes.') },
        { id: 'foley-artist',                   label: 'Foley Artist',                  credits: credOf('foley-artist'),                   input: build('Design foley plan — footsteps, cloth movement, prop handling, environmental foley per scene. Return foley_plan[] with scene_ref, foley_elements[], priority.') },
        { id: 'composer',                       label: 'Composer',                      credits: credOf('composer'),                       input: build('Design score strategy — themes per character/location, instrumentation, emotional arc of music, cue placements. Return score_strategy with themes[], instrumentation, cue_plan[].') },
        { id: 'music-sync-specialist',          label: 'Music Sync Specialist',         credits: credOf('music-sync-specialist'),          input: build('Design music sync plan — licensed track placements, sync points, mood alignment, licensing tier recommendations. Return sync_plan[] with scene_ref, track_suggestion, sync_point, mood.') },
        { id: 'ambient-sound-designer',         label: 'Ambient Sound Designer',        credits: credOf('ambient-sound-designer'),         input: build('Design ambient soundscape per scene — room tone, environmental ambience, off-screen world sounds. Return ambient_plan[] with scene_ref, soundscape, key_elements[].') },
        { id: 'sound-design-lead',              label: 'Sound Design Lead',             credits: credOf('sound-design-lead'),              input: build('Synthesize full sound design vision — signature sounds, sonic motifs, sound-to-picture philosophy. Return sound_design with motifs[], philosophy, signature_sounds[].') },
        { id: 'mixer',                          label: 'Mixer',                         credits: credOf('mixer'),                          input: build('Design mix strategy — dialogue/music/SFX balance, dynamic range, loudness target, stem structure. Return mix_strategy with balance_notes, loudness_target, stem_plan[].') },
        { id: 'sound-effects-librarian',        label: 'Sound Effects Librarian',       credits: credOf('sound-effects-librarian'),        input: build('Catalog all required sound effects — categorize by type, source recommendation, priority. Return sfx_catalog[] with name, type, scene_refs[], source_recommendation, priority.') },
        { id: 'emotional-audio-enhancer',       label: 'Emotional Audio Enhancer',      credits: credOf('emotional-audio-enhancer'),       input: build('Design emotional audio enhancement per scene — subliminal bass, binaural elements, silence as tool, tension-building techniques. Return emotional_audio[] with scene_ref, technique, emotional_target, implementation_notes.') },
      ];
    }
    case 'deliver': {
      const build = (instr) => JSON.stringify({...commonScriptInput, shot_list: p.shot_list, timeline: p.timeline, instruction: instr}, null, 2);
      return [
        { id: 'executive-producer',           label: 'Executive Producer',           credits: credOf('executive-producer'),           input: build('Final executive review — assess overall project quality, commercial viability, and creative integrity. Return exec_verdict, quality_score (1-10), commercial_notes, creative_integrity_notes, greenlight_recommendation.') },
        { id: 'showrunner',                   label: 'Showrunner',                   credits: credOf('showrunner'),                   input: build('Showrunner final pass — verify all creative decisions are coherent end-to-end, flag any last-minute continuity or tone issues. Return showrunner_verdict, coherence_score, final_flags[], sign_off.') },
        { id: 'line-producer',                label: 'Line Producer',                credits: credOf('line-producer'),                input: build('Compile production summary — total shot count, scene count, estimated render time, VFX complexity summary, delivery readiness. Return production_summary with counts, estimates, readiness_score.') },
        { id: 'production-manager',           label: 'Production Manager',           credits: credOf('production-manager'),           input: build('Final production checklist — verify all assets are accounted for, all agents have run, all action items resolved. Return checklist[] with item, status, blocking_issues[].') },
        { id: 'post-production-supervisor',   label: 'Post-Production Supervisor',   credits: credOf('post-production-supervisor'),   input: build('Post-production delivery plan — codec specs, resolution, color space, audio specs, naming conventions, delivery platform requirements. Return post_plan with specs, naming_convention, platform_requirements[].') },
        { id: 'export-delivery-specialist',   label: 'Export & Delivery Specialist', credits: credOf('export-delivery-specialist'),   input: build('Compile final export manifest — all clips, their status, render order, stitching sequence, output format. Return export_manifest[] with clip_ref, status, render_order, output_format.') },
        { id: 'version-control-agent',        label: 'Version Control',              credits: credOf('version-control-agent'),        input: build('Snapshot the current project state — summarize all changes made across all agents, create a version label, list what changed from the previous version. Return version_snapshot with label, summary, changes_from_prior[].') },
        { id: 'project-memory-keeper',        label: 'Project Memory Keeper',        credits: credOf('project-memory-keeper'),        input: build('Extract and preserve key creative decisions made throughout production — vision choices, character decisions, world rules, style locks. Return memory_document with creative_decisions[], world_rules[], style_locks[], character_canon[].') },
        { id: 'character-bible-maintainer',   label: 'Character Bible Maintainer',   credits: credOf('character-bible-maintainer'),   input: build('Compile final character bible — all characters with canonical descriptions, voice signatures, arc summaries, wardrobe, and continuity rules. Return character_bible[] with all fields per character.') },
        { id: 'reference-image-curator',      label: 'Reference Image Curator',      credits: credOf('reference-image-curator'),      input: build('Compile reference image requirements — per character, per location, per visual style. Generate detailed image prompt for each reference needed. Return reference_list[] with type, subject, prompt, priority.') },
        { id: 'feedback-integrator',          label: 'Feedback Integrator',          credits: credOf('feedback-integrator'),          input: build('Synthesize all agent outputs into a final project health report. Score each department. Identify the top 3 remaining risks. Return final_report with department_scores{}, top_risks[], overall_score, delivery_recommendation.') },
      ];
    }
    case 'quality': {
      const build = (instr) => JSON.stringify({...commonScriptInput, shot_list: p.shot_list, instruction: instr}, null, 2);
      return [
        { id: 'continuity-supervisor',           label: 'Continuity Supervisor',           credits: credOf('continuity-supervisor'),           input: build('Full continuity audit — props, wardrobe, hair/makeup, set dressing, lighting, time-of-day across all scenes. Return continuity_flags[] with scene_ref, element, issue, severity.') },
        { id: 'wardrobe-prop-auditor',            label: 'Wardrobe & Prop Auditor',         credits: credOf('wardrobe-prop-auditor'),            input: build('Audit wardrobe and prop continuity across all scenes — flag inconsistencies, anachronisms, character-costume mismatches. Return audit_flags[] with scene_ref, element, issue, fix_suggestion.') },
        { id: 'geography-set-guardian',           label: 'Geography & Set Guardian',        credits: credOf('geography-set-guardian'),           input: build('Audit spatial logic across all scenes — screen direction, geography consistency, set layout continuity. Return geography_flags[] with scene_ref, issue, severity, fix_suggestion.') },
        { id: 'performance-consistency-checker',  label: 'Performance Consistency Checker', credits: credOf('performance-consistency-checker'),  input: build('Audit character performance consistency — emotional state continuity, physical behavior patterns, accent/voice consistency across scenes. Return performance_flags[] with scene_ref, character, issue, severity.') },
        { id: 'dramatic-logic-guardian',          label: 'Dramatic Logic Guardian',         credits: credOf('dramatic-logic-guardian'),          input: build('Final dramatic logic audit — unmotivated decisions, plot holes, cause-effect failures. Return logic_flags[] with scene_ref, issue, severity, suggested_fix.') },
        { id: 'visual-consistency-guardian',      label: 'Visual Consistency Guardian',     credits: credOf('visual-consistency-guardian'),      input: build('Final visual consistency audit — lighting continuity, color grade consistency, lens/framing rule adherence. Return visual_flags[] with scene_ref, element, issue, severity.') },
        { id: 'emotional-truth-guardian',         label: 'Emotional Truth Guardian',        credits: credOf('emotional-truth-guardian'),         input: build('Audit emotional truth across all scenes — verify character emotional states are earned, transitions are motivated, climaxes land. Return emotional_flags[] with scene_ref, character, issue, severity.') },
        { id: 'timeline-consistency-checker',     label: 'Timeline Consistency Checker',    credits: credOf('timeline-consistency-checker'),     input: build('Final timeline audit — verify chronological logic, flashback/flash-forward clarity, time-skip coherence. Return timeline_flags[] with scene_ref, issue, severity.') },
      ];
    }
    default:
      return [];
