// atlas/workers/src/handlers/quote.ts
import type { Env, Quote } from '../env';

// Quote provider — Polygon preferred (real-time), Finnhub fallback (delayed)
const KV_PREFIX = 'q:';
const TTL_OPEN_MS = 30_000;    // 30 seconds during market hours
const TTL_CLOSED_MS = 300_000; // 5 minutes when market is closed
...
/**
 * Get single stock quote with aggressive caching
 */
export async function getQuote(env: Env, ticker: string, opts: { force?: boolean } = {}): Promise<Quote | null> {
  ...
  if (env.POLYGON_API_KEY) {
    q = await fetchPolygonQuote(env, tk);
  }
  // Fallback to Finnhub
  if (!q && env.FINNHUB_KEY) {
    q = await fetchFinnhubQuote(env, tk);
  }
...
/**
 * Batch quote fetch — optimized for daily analysis
 */
export async function getQuotesBatch(env: Env, tickers: string[]): Promise<Record<string, Quote>> {
  ...
  // Polygon batch snapshot (very fast)
  if (env.POLYGON_API_KEY) {
  ...
  // Finnhub fallback (rate-limited, so chunked)
  if (env.FINNHUB_KEY) {
    ...
    for (let i = 0; i < stillMissing.length; i += 6) {
      ...
      results.forEach((q, j) => {
        if (q) {
          out[chunk[j]!] = q;
          const ttl = isMarketOpen() ? TTL_OPEN_MS : TTL_CLOSED_MS;
          env.CACHE.put(`${KV_PREFIX}${chunk[j]!}`, JSON.stringify(q), { expirationTtl: Math.ceil(ttl / 1000) }).catch(() => {});
        }
      });
      if (i + 6 < stillMissing.length) await new Promise(r => setTimeout(r, 200)); // respect rate limit
    }
  }
...
async function fetchFinnhubQuote(env: Env, ticker: string): Promise<Quote | null> {
  try {
    ...
    return {
      ...
      ts: Date.now(),
      source: 'finnhub',
      delayed: true, // free tier is delayed
    };
  } catch {
    return null;
  }
