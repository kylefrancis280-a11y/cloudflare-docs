// atlas/workers/src/handlers/news.ts
...
const TTL_NEWS_SEC = 1800;   // 30 minutes
const TTL_RECS_SEC = 21600;  // 6 hours
...
/**
 * Get recent news for a ticker (cached 30 min)
 */
export async function getNews(env: Env, ticker: string, limit = 6): Promise<NewsItem[]> {
  ...
  if (env.POLYGON_API_KEY) {
    items = await fetchPolygonNews(env, tk, limit);
  }
  // Fallback to Finnhub
  if (!items.length && env.FINNHUB_KEY) {
    items = await fetchFinnhubNews(env, tk, limit);
  }
...
/**
 * Get latest analyst recommendations (cached 6 hours)
 */
export async function getRecs(env: Env, ticker: string): Promise<AnalystRecs | null> {
  ...
  if (env.FINNHUB_KEY) {
    r = await fetchFinnhubRecs(env, tk);
  }
...
async function fetchPolygonNews(env: Env, ticker: string, limit: number): Promise<NewsItem[]> {
  try {
    ...
    return (data?.results ?? []).slice(0, limit).map((n: any) =>
      sanitizeNewsItem({
        headline: n.title ?? '',
