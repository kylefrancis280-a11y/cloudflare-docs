// atlas/workers/src/handlers/backtest.ts
import type { Env } from '../env';
import { getQuote } from '../data/quote';
import { todayET } from '../lib/http';

/**
 * Backfill 1-day returns for yesterday's AI analysis calls.
 * Runs nightly to calculate how accurate our sentiment calls were.
 */
export async function backfillReturns(env: Env, lookbackDays = 30): Promise<{ updated: number }> {
  const today = todayET();
  const cutoff = new Date(Date.now() - lookbackDays * 86_400_000).toISOString().split('T')[0];

  // Find analysis entries that don't have a 1d return recorded yet
  const rows = await env.DB.prepare(
    `SELECT a.date, a.ticker, a.strength, a.sentiment
     FROM analysis a
     LEFT JOIN backtest_returns b ON b.date = a.date AND b.ticker = a.ticker
     WHERE a.date >= ? AND a.date < ? AND b.ret_1d IS NULL
     ORDER BY a.date DESC LIMIT 500`
  ).bind(cutoff, today).all<{ date: string; ticker: string }>();

  let updated = 0;

  for (const r of (rows.results ?? [])) {
    const quote = await getQuote(env, r.ticker);
    if (!quote?.pct) continue;

    await env.DB.prepare(
      `INSERT INTO backtest_returns (date, ticker, ret_1d)
       VALUES (?, ?, ?)
       ON CONFLICT(date, ticker) DO UPDATE 
       SET ret_1d = excluded.ret_1d,
           computed_at = strftime('%Y-%m-%dT%H:%M:%fZ','now')`
    ).bind(r.date, r.ticker, quote.pct).run();

    updated++;
  }

  return { updated };
}

/**
 * Calculate historical track record by sentiment band.
 * Shows users real performance stats like "Bullish calls hit 62% of the time".
 */
export async function trackRecord(env: Env, days = 30): Promise<{
  sentiment: string;
  n: number;
  hitRate: number;
  avgRet: number;
}[]> {
  const cutoff = new Date(Date.now() - days * 86_400_000).toISOString().split('T')[0];

  const rows = await env.DB.prepare(
    `SELECT 
       a.sentiment,
       COUNT(*) AS n,
       AVG(CASE 
         WHEN (a.sentiment IN ('Strongly Bullish','Bullish') AND b.ret_1d > 0)
           OR (a.sentiment IN ('Cautious','Bearish') AND b.ret_1d < 0)
           OR (a.sentiment = 'Neutral' AND ABS(b.ret_1d) < 1)
         THEN 1.0 ELSE 0.0 
       END) AS hit_rate,
       AVG(b.ret_1d) AS avg_ret
     FROM analysis a
     JOIN backtest_returns b ON b.date = a.date AND b.ticker = a.ticker
     WHERE a.date >= ? AND b.ret_1d IS NOT NULL
     GROUP BY a.sentiment`
  ).bind(cutoff).all<{ sentiment: string; n: number; hit_rate: number; avg_ret: number }>();

  return (rows.results ?? []).map(r => ({
    sentiment: r.sentiment,
    n: r.n,
    hitRate: +(r.hit_rate * 100).toFixed(1),
    avgRet: +(r.avg_ret).toFixed(2),
  }));
}
