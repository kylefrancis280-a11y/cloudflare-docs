// atlas/workers/src/handlers/quotes.ts
...
/**
 * GET /api/quote?t=NVDA
 * Single ticker quote (Polygon preferred, Finnhub fallback)
 */
export async function handleQuote(req: Request, env: Env): Promise<Response> {
  const ip = clientIp(req);
  const limit = await rateLimit(env, { key: `quote:${ip}`, max: 120, windowMs: 60_000 });
  if (!limit.ok) return err(429, 'rate limit exceeded', req, env, { retryAfterSec: limit.retryAfterSec });

  const u = new URL(req.url);
  const ticker = u.searchParams.get('t')?.toUpperCase();

  if (!ticker) return err(400, 'ticker required (use ?t=NVDA)', req, env);
  if (!TICKER_RE.test(ticker)) return err(400, 'invalid ticker format', req, env);

  const q = await getQuote(env, ticker);
  if (!q) return err(404, 'no quote available for this ticker', req, env);
...
/**
 * GET /api/quotes?t=NVDA,AAPL,TSLA
 * Batch quotes (used by dashboard and daily analysis)
 */
export async function handleQuotesBatch(req: Request, env: Env): Promise<Response> {
  const ip = clientIp(req);
  const limit = await rateLimit(env, { key: `quotes:${ip}`, max: 60, windowMs: 60_000 });
  if (!limit.ok) return err(429, 'rate limit exceeded', req, env, { retryAfterSec: limit.retryAfterSec });
  ...
  if (param) {
    ...
    // Default: today's AI-ranked featured stocks (from rankings table)
    ...
    for (const r of (rows.results ?? [])) {
      try {
        (JSON.parse(r.payload_json) as Array<{ ticker: string }>).forEach(f => set.add(f.ticker));
      } catch {}
    }

    if (!set.size) {
      // Fallback: any active stocks
