// atlas/workers/src/handlers/admin.ts
 ...
 * - Creating additional staff (admin/analyst)
 * - Force password reset
 * - Listing users & audit log
 */
export async function handleAdmin(req: Request, env: Env): Promise<Response> {
  ...
  // ── Bootstrap mode (only allowed when no admins exist yet) ─────────────
  ...
  if (!ctx) {
    ...
    if (!token || !expected || token !== expected) {
      return err(401, 'auth required', req, env);
    }

    const adminCount = await env.DB.prepare(`SELECT COUNT(*) AS n FROM users WHERE role = 'admin'`)
      .first<{ n: number }>();

    if ((adminCount?.n ?? 0) > 0) {
      return err(403, 'admin already exists — bootstrap disabled', req, env);
    }
  ...
  switch (action) {
    case 'create-staff': {
      ...
      if (!['admin', 'analyst'].includes(role)) {
        return err(400, 'role must be admin or analyst', req, env);
      }
      if (!email || !name || password.length < 16) {
        return err(400, 'email, name required + password >= 16 chars', req, env);
      }

      const existing = await env.DB.prepare(`SELECT id FROM users WHERE email = ?`).bind(email).first();
      if (existing) return err(409, 'user already exists', req, env);
      ...
      await env.DB.prepare(`
        INSERT INTO users (
          id, email, name, password_hash, password_salt, password_iters,
    ...
    case 'force-reset-password': {
      ...
      if (!email || password.length < 16) {
        return err(400, 'email + password (>=16 chars) required', req, env);
      }
    ...
    case 'list-users': {
      const rows = await env.DB.prepare(`
        SELECT id, email, name, role, tier, subscription_status, created_at 
        FROM users 
    ...
    default:
      return err(400, 'unknown action', req, env);
  }
}
