// atlas/workers/src/handlers/stripe.ts
 ...
 * Rejects anything that isn't properly signed with STRIPE_WEBHOOK_SECRET
 */
...
function amountToTier(cents: number): Tier {
  if (cents >= 9500) return 'institutional'; // CA$99.99
  if (cents >= 4500) return 'pro';           // CA$49.99
  if (cents >= 2500) return 'core';          // CA$29.99
  return 'core';
}
...
async function updateUser(
  ...
  // Update by customer ID as fallback
  if (customerId) {
    const existing = await env.DB.prepare(`SELECT id FROM users WHERE stripe_customer_id = ? LIMIT 1`)
      .bind(customerId)
      .first<{ id: string }>();
    ...
    if (existing) {
      await env.DB.prepare(`
        ...
        SET subscription_status = ?,
            ...
            updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now')
        WHERE id = ?
      `).bind(status, tier, existing.id).run();
      return;
    }
  }

  // No matching user yet — log for manual backfill
  await env.DB.prepare(`
    INSERT INTO audit_log (user_id, action, details)
    VALUES (NULL, 'stripe_unmatched_event', json_object('email', ?, 'customer', ?, 'status', ?, 'tier', ?))
  `).bind(normEmail, customerId, status, tier).run().catch(() => {});
}
