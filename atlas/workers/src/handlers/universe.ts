// atlas/workers/src/handlers/universe.ts
import type { Env } from '../env';
import { json } from '../lib/http';

/**
 * Static sector metadata used by both backend and frontend
 */
...
/**
 * GET /api/universe
 * Returns the full active stock universe grouped by sector with metadata.
 * Cached aggressively on the frontend (1 hour + stale-while-revalidate).
 */
export async function handleUniverse(req: Request, env: Env): Promise<Response> {
  const rows = await env.DB.prepare(
    `SELECT ticker, name, exchange, industry, sector, pe, market_cap 
     FROM universe 
     WHERE active = 1 
  ...
  const sectors: Record<string, {
    meta: typeof SECTOR_META[string];
    stocks: typeof rows.results;
  }> = {};
