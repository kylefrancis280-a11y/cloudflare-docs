// /.netlify/functions/get-daily
// Returns today's Grok-generated daily analysis from Firebase
...
exports.handler = async (event) => {
  ...
  if (!SECRET) return cors(500, { error: 'Server not configured — set FIREBASE_DB_SECRET' });

  try {
    const params = event.queryStringParameters || {};
    const date = params.date || new Date().toISOString().split('T')[0];

    // Get specific ticker analysis
    if (params.ticker) {
